...
Read `docs/references/component-inventory.md` before adding components. The inventory contains 46 normalized families in seven sequential dependency-safe chunks. The current library scope is only the implemented five-family pilot under `src/components/ui/`: Button, Input, Textarea, Select, and a self-contained native ARIA editable Combobox built with Input and Button. All five are technically verified and await user visual approval. Every later family is pending approval and must not be implemented.
