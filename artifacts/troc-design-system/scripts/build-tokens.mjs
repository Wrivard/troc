...
function hexToHslChannels(hex) {
  ...
  const H = Number((hue * 360).toFixed(5));
  const S = Number((s * 100).toFixed(5));
  const L = Number((l * 100).toFixed(5));
  return `${H} ${S}% ${L}%`;
}
...
function buildFavicon(tokens) {
  const leaf = readFileSync(join(root, "public/brand/troc-leaf.png")).toString("base64");
  return `<svg width="180" height="180" viewBox="0 0 180 180" fill="none" xmlns="http://www.w3.org/2000/svg">
  <image href="data:image/png;base64,${leaf}" x="0" y="0" width="180" height="180" preserveAspectRatio="xMidYMid meet"/>
</svg>
`;
}
...
function buildCss(tokens) {
  ...
  const vars = (group) => Object.entries(group).filter(([name]) => !name.startsWith("$"))
    .map(([name, node]) => `  --${name}: ${resolveValue(node, tokens)};`).join("\n");
  // CSS custom properties resolve before inheritance. Re-declare aliases at
  // each theme boundary so nested comparison panels do not inherit root colors.
  const scopedColors = (scope) => Object.entries(colorEntries(scope, tokens)).map(([name, hex]) => {
    const role = name.replace(/([A-Z])/g, "-$1").replace(/(\d+)/g, "-$1").toLowerCase();
    return `  --${role}: ${hexToHslChannels(hex)};\n  --color-${role}: ${hex};`;
  }).join("\n");
  css += `\n:root, .light {\n${scopedColors("light")}\n}\n.dark {\n${scopedColors("dark")}\n}\n`;
  css += `\n:root, .light {\n${vars(tokens.semantic.light)}\n}\n.dark {\n${vars(tokens.semantic.dark)}\n}\n`;
  const foundation = [];
  for (const [group, entries] of Object.entries(tokens.foundation)) {
    for (const [name, node] of Object.entries(entries)) {
      foundation.push(`  --${group}-${name}: ${resolveValue(node, tokens)};`);
    }
  }
  css += `\n:root {\n${foundation.join("\n")}\n}\n`;
  for (const [name, node] of Object.entries(tokens.typography.scale)) {
    const value = resolveValue(node, tokens);
    css += `.type-${name} { font-size: ${value.size}; line-height: ${value.lineHeight}; font-weight: ${value.weight}; letter-spacing: ${value.tracking}; }\n`;
  }
  return css;
}
...
function buildTs(tokens) {
  const portable = {
    ...
    semantic: Object.fromEntries(["light", "dark"].map((scope) => [scope,
      Object.fromEntries(Object.entries(tokens.semantic[scope]).map(([key, value]) => [key, resolveValue(value, tokens)])),
    ])),
    foundation: Object.fromEntries(Object.entries(tokens.foundation).map(([group, entries]) => [group,
      Object.fromEntries(Object.entries(entries).map(([key, value]) => [key, resolveValue(value, tokens)])),
    ])),
    typeScale: Object.fromEntries(Object.entries(tokens.typography.scale).map(([key, node]) => [key, resolveValue(node, tokens)])),
  };
