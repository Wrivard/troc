...
export const CATALOG_IMAGES_SQL = `WITH approved AS (SELECT i.product_id,i.variant_id,i.position,jsonb_build_object(
      ...
      JOIN LATERAL (SELECT (jsonb_agg(url ORDER BY width DESC)->>0) AS url,
        jsonb_agg(jsonb_build_object('url',url,'width',width) ORDER BY width) AS sources
        FROM troc.catalog_image_renditions WHERE image_id=i.id HAVING count(*)>0) renditions ON true
...
export class PostgresCatalogAssetProvider implements CatalogAssetProvider {
  ...
  async images(products: Product[]): Promise<Product[]> {
    ...
    if (products.length > MAX_CATALOG_PAGE_SIZE)
      throw new Error("asset_batch_too_large");
    const { rows } = await this.db.query<{
      product_id: string;
      variant_id: string | null;
      image: CatalogImage;
    }>(CATALOG_IMAGES_SQL, [products.map((p) => p.id)]);
