...
try{
 ...
 for(const q of ["","Northern Card 09999","09999/10000","maple aurora","Carte Boréale 09999","オーロラ","no-such-product-zzzz"]){
   ...
   for(let i=0;i<5;i++){const start=performance.now();const page=await repo.search(filtersFrom(new URLSearchParams({q,limit:"12"})));durations.push(performance.now()-start);found=page.items.map(r=>r.product.slug);}
 ...
 await writeFile("docs/evidence/search/after-paging.json",JSON.stringify(report,null,2));
