...
## Seller UX follow-up V01–V04

Role changes require an explicit selection; removing access still sends the existing null role contract. Current seller navigation exposes aria-current and pagination is hidden when the first page is shorter than the existing 50-row bound. Account-ID help distinguishes IDs from email addresses; the Account page retrieval location awaits Design/A integration. Initial and workspace errors now distinguish sign-in, permission denial, and temporary service/network failures in EN/FR. No permission, API or migration changes.

Author validation: 16 mocked initial/workspace access-error browser cases, 8 review UI cases (including explicit choice and remove/owner payloads), 8 pagination cases, and 8 actual-database integrated seller workflow cases passed. Marketplace typecheck, scoped lint and seller production build passed. Independent C/UX retest is required for this follow-up; the earlier combined dfd58eb local PASS does not automatically cover it. Staging authorization is recorded by the coordinator; hosted/PostgreSQL gates remain open until evidence is collected.
